...
//leaf_page* find_leaf;//for_db_find
//page_t* find_page;//for_db_find

/*
void bpt_set()
{
	find_leaf = (leaf_page*)malloc(Page_size);
	find_page = (page_t*)malloc(Page_size);
}
...
void print_leaves()
{
	...
	{
		file_read_page(root_number,page);
...
int db_find(int64_t key,char* ret_values)
{
	uint64_t root_number = 0;
	internal_page* internal = (internal_page*)malloc(Page_size);
	...
	{
		file_read_page(root_number,find_page);
...
int Start_new_tree(record* rec)
{
	...
	//printf("problem-1\n");
	file_read_page(root_number,tmp);
	...
	file_write_page(root_number,tmp);
	free(leaf);
	free(tmp);
...
int insert_record_into_leaf_split(pagenum_t old_number,record* rec)
{
	...
	file_read_page(new_number,new_page);
	...
	//return 0;
	return insert_into_parent(parent_number,old_number,new_key,new_number,1);

	
}

int insert_into_parent(pagenum_t parent_number,pagenum_t old_number, int64_t new_key,pagenum_t new_number,int leaf_check)
{
	...
	else // child_page have parent_page
	{
		...
		left_index = get_left_index(parent_number,new_key); // find appropriate position new key

		if(internal->Number_of_Keys<Internal_factor-1)// parent not full
		{
			return_number = insert_key_into_internal(parent_number,new_key,new_number,left_index);
			
		}
		else if(internal->Number_of_Keys==Internal_factor-1) //parent full
		{
			return_number = insert_key_into_internal_split(parent_number,new_key,new_number,left_index);
		}
...
int insert_key_into_internal_split(pagenum_t old_number,int64_t new_key,pagenum_t new_number,int left_index)
{
	...
	return insert_into_parent(parent_number,old_number,k_prime_key,new_internal_number,0);
...
int db_insert(int64_t key,char* values)
{
	...
	{	
		return_number = Start_new_tree(rec);
	}
	else
	{
		page_t* page;
		...
		if(duplicate==0) // no same key in page
		{
			...
			{
				return_number = insert_record_into_leaf_split(page_number,rec);
			}
			else if(leaf->Number_of_Keys<Leaf_factor-1) // non-full leaf page
			{
				return_number = insert_record_into_leaf(page_number,rec);
			}
...
int delete_in_leaf(int64_t key,pagenum_t leaf_number)
{
	...
	else if(leaf->Number_of_Keys==0) // leaf is empty, so we delete empty leaf page(delayed_merge)
	{
		...
		else if(parent_number!=0) // go parent page and delete leaf_page
		{
			free(leaf);
			free(page);
			return delayed_merge_leaf(parent_number,key_prime);
		}
	}
}

int delayed_merge_leaf(pagenum_t parent_number,int64_t prime_key)//leaf(child)->internal(parent) case
{
	...
	else if(parent_internal->Number_of_Keys==0) // we delete parent_page
	{
		...
		else
		{
			...
			if(uncle_internal->Number_of_Keys==Internal_factor-1) // oh, uncle is full
			{
				...
				free(parent_internal);
				free(parent_page);
				return delayed_redistribute_internal(grand_parent_number,key_prime,uncle_number);
			...
			else if(uncle_internal->Number_of_Keys<Internal_factor-1) // oh, uncle is not full
			{
				...
				free(parent_internal);
				free(parent_page);
				return delayed_merge_internal(grand_parent_number,key_prime,prime_page_num);
...
int delayed_merge_internal(pagenum_t parent_number, int64_t prime_key,pagenum_t prime_pagenum)//internal(child)->internal(parent)
{
	pagenum_t deleted_number, prime_page_num;
	int64_t key_prime;
	int i, deleted_index, grand_parent_number;
...
//functions about translate page<->internal,leaf
